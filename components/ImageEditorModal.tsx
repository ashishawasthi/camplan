...
const ImageEditorModal: React.FC<Props> = ({ creative, onClose, onSave, setError }) => {
  ...
  const handleEdit = async () => {
    ...
    try {
      const base64Data = creative.imageUrl.split(',')[1];
  ...
  const handleSave = () => {
    if (editedImageUrl && editedMimeType) {
      onSave({
        ...
        prompt: `${creative.prompt} (Edited: ${prompt})`
      });
    }
  };
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div>
              ...
              <img src={creative.imageUrl} alt="Original Creative" className="w-full rounded-lg" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-600 mb-2">Edited</h3>
              <div className="w-full aspect-square bg-slate-100 rounded-lg flex items-center justify-center border-2 border-dashed border-slate-200">
                {isEditing ? (
                  <div className="flex flex-col items-center">
                    <SparklesIcon className="h-8 w-8 text-indigo-500 animate-pulse" />
                    <p className="mt-2 text-sm text-slate-500">Applying edit...</p>
                  </div>
                ) : editedImageUrl ? (
                  <img src={editedImageUrl} alt="Edited Creative" className="w-full rounded-lg" />
                ) : (
                  <p className="text-slate-500 text-sm p-4 text-center">Your edited image will appear here.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <input
              ...
              className="flex-grow block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            />
